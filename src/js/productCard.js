...
element.addEventListener("click", function () {
  if (block.classList.contains("product__main-share-block--open")) {
    block.classList.remove("product__main-share-block--open");
  } else {
    block.classList.add("product__main-share-block--open");
  }
});
...
document.querySelector(".product__main-more").addEventListener("click", () => {
  ...
  if (description.classList.contains("product__description--is-active")) {
    ...
    document.querySelector(".product__main-more").textContent =
      "Все характеристики";
  }
});
